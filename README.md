...
- Real portal-based cell visibility: an adjacent room renders as soon as it's actually visible through an open doorway (a camera-frustum test walked across each room's real portal connections), not only once physically entered.
- Real terrain grading around player structures — confirmed, by reading the actual server-side placement logic, that the live game applies essentially no terrain grading to player housing at all; this project now matches that real behavior exactly instead of guessing at a "fix" for a seam the real game itself has.
...
- Real skeletal animation: keyframe playback driven by the client's own gender/mood-aware animation-state selection format, replacing static bind-pose rendering, now also incorporating real per-creature context (posture, mood, held weapon, combat/sitting/mounted state). The resting stance has a known, thoroughly-investigated, still-open visual defect — see below and `PROGRESS.md`.
...
- **An open call for help:** the character's resting stance holds its arms out and away from the body instead of at its sides, unlike the real client. This is the most thoroughly investigated open defect in this project's history — every per-clip animation data source both relevant asset file formats can contain has been read, checked directly against the real client's own runtime behavior, and, where a real gap existed, fixed (see `PROGRESS.md`) — and none of it explains the difference. If you have real hands-on knowledge of this game's animation pipeline or rest-pose conventions and recognize this symptom, please open an issue.
- The walk animation's stride length doesn't yet match the real client (reaches noticeably farther than it should) — narrowed down via direct side-by-side comparison against real official-client footage and cross-validated against an independent third-party implementation; the recorded animation data itself appears to genuinely contain this range of motion, very likely not a bug in this project's own code. See `PROGRESS.md`.
- A real guildhall's entrance ramp still lets the character fall through terrain right at its outer edge — the building's own floor-collision mesh doesn't cover the ramp's full real length, and several attempted fixes haven't fully closed the gap yet. See `PROGRESS.md`.
